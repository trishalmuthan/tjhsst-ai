...
def random_with_replacement(TRAINING):
    small_training = list()
    for i in range(int(len(TRAINING)/10.25)):
        small_training.append(random.choice(TRAINING))
    #print(len(small_training))
    return small_training

def classify_rf(data_value, heads):
    classifications = dict()
    for head in heads:
        curr_class = classify(data_value, head)
        if curr_class in classifications.keys():
            classifications[curr_class] += 1
        else:
            classifications[curr_class] = 1
    final_class = max(classifications, key=classifications.get)
    #print(final_class)
    return final_class

#given a list of features, data to extract a training set from, a test set, and other logistic info, creating a learning curve
#return type: [(size, accuracy), ...]
def create_rf_learning_curve(features, dataset, TEST, min_train_size, max_train_size, train_step):
    accuracies = list()
    for SIZE in range(min_train_size, max_train_size, train_step):
        #print(SIZE)
        heads = list()
        #randomly create entire training dataset
        TRAINING = random.sample(dataset, SIZE)
        while not all_same_classification(TRAINING):
            TRAINING = random.sample(dataset, SIZE)
        for i in range(10):
            head = generate_tree(features, random_with_replacement(TRAINING), Node('Start', -1, -1, False, False, [], True, None))
            heads.append(head)
        num_correct = 0
        for test_value in TEST:
            if classify_rf(test_value, heads) == test_value[-1]:
                num_correct += 1
        accuracies.append((SIZE, num_correct/len(TEST)))
    return accuracies
...
accuracies = create_rf_learning_curve(features, dataset[:-test_size], TEST, min_train_size, max_train_size, train_step)
